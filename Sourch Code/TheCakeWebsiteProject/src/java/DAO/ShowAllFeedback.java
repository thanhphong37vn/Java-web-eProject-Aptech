...
public class ShowAllFeedback extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        ResultSet rs = null;
        try {
            stm = conn.prepareStatement("SELECT * FROM Feedback");
            rs = stm.executeQuery();
            while (rs.next()) {
                FeedBack fd = new FeedBack();
                fd.setFeedBackID(rs.getInt("FeedBackId"));
                fd.setName(rs.getString("Name"));
                fd.setSubject(rs.getString("Email"));
                fd.setContent(rs.getString("Subject"));
                fd.setEmail(rs.getString("Content"));

                fd.setFeedbackDate(rs.getString("FeedbackDate"));
                list.add(fd);
            }
