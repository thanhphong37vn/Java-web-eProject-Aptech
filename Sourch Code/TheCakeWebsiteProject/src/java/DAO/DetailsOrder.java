...
public class DetailsOrder extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        ResultSet rs = null;
        try {
            stm = conn.prepareStatement("SELECT * FROM DetailOrders where OrdersId=?");
            stm.setString(1, id);
            rs = stm.executeQuery();
            while (rs.next()) {

                DetailOrders de=new DetailOrders();
                de.setId(rs.getString("ID"));
                de.setOrdersId(rs.getInt("OrdersId"));
                de.setProductID(rs.getInt("ProductID"));
                de.setQuantity(rs.getInt("Quantity"));

                request.setAttribute("displaydetailsOrder", de);
            }
