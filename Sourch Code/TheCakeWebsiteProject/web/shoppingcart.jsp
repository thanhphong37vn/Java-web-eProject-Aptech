        ...
        <div id="content">
            <div class="home">
                ...
                <c:if test="${not empty shop}">
                    <h1>
                        ${requestScope['count']} product(s) in Cart
                        with  <fmt:formatNumber type="currency" value="${requestScope['totalPriceInCart']}"/>
                    </h1>
                    <table border="1" width="100%">
                        ...
                        <tbody>
                            ...
                            <c:forEach var="row" items="${requestScope['cartList']}" varStatus="indexHere">
                                <tr>
                                    <td>${indexHere.index+1}</td>

                                    <td>${row.proName}</td>
                                    <td> <fmt:formatNumber type="currency" value="${row.price}"/></td>
                            <td>${row.noOfProduct}</td>
                            <td>
                            <fmt:formatNumber type="currency" value="${row.totalPrice}"/>
                            </td>
                            <td>
                                <a href="Controller?action=Remove&proId=${row.proId}">Delete</a>
                            </td>
                            </tr>
                        </c:forEach>
                        </tbody>
                    ...
                    <hr/>
                    <form action="UpdateOrderfromUser">
                        <button type="submit" >      <a class="view">Send Order</a> </button>
                    </form>
